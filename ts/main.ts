...
const $favorites = document.getElementById('favs-link') as HTMLAnchorElement;
...
async function fetchCharacters(): Promise<Character[]> {
  try {
    ...
    const resultMatches = characters.filter((character: any) =>
...
function addToFavorites(characterId: string, characters: Character[]): void {
...
function createCharacterCard(
  ...
  if (!character.image) {
    img.setAttribute(
      'src',
      'https://upload.wikimedia.org/wikipedia/en/thumb/b/b1/Portrait_placeholder.png/320px-Portrait_placeholder.png',
    );
  } else {
  ...
  addToFavs.addEventListener('click', () =>
    addToFavorites(character.id, characters),
  );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  for (let i = 0; i < data.favorites.length; i++) {
    $favoritesPage?.appendChild(
      createCharacterCard(data.favorites[i], data.favorites),
    );
  }
});
